...
abstract class AbstractAnimatedDrawingState extends State<AnimatedDrawing> {
  ...
  void evokeOnPaintForNewlyPaintedPaths(int currentPaintedPathIndex) {
    final paintedPaths = pathSegments[currentPaintedPathIndex].pathIndex - lastPaintedPathIndex; //TODO you should iterate over the indices of the sorted path segments not the original ones
  ...
  bool newPathPainted(int currentPaintedPathIndex) {
    return widget.onPaint != null && currentPaintedPathIndex != -1 && pathSegments[currentPaintedPathIndex].pathIndex - lastPaintedPathIndex > 0;
  }
  ...
  void assignPathSegmentsToPainters() {
    ...
    if (widget.range != range) {
      ...
      pathSegmentsToAnimate = pathSegments.where((x) => (x.pathIndex >= widget.range!.start! && x.pathIndex <= widget.range!.end!)).toList();
  ...
  void checkValidRange() {
    RangeError.checkValidRange(widget.range!.start!, widget.range!.end, widget.paths.length - 1, 'start', 'end', 'The provided range is invalid for the provided number of paths.');
  }
  ...
  CustomPaint createCustomPaint(BuildContext context) {
    ...
    return CustomPaint(foregroundPainter: buildForegroundPainter(), painter: buildBackgroundPainter(), size: Size.copy(MediaQuery.of(context).size));
  }
  ...
  bool checkIfDefaultOrderSortingRequired() {
    ...
    final defaultSortingWhenNoOrderDefined = widget.lineAnimation == LineAnimation.allAtOnce && animationOrder != PathOrders.original;
